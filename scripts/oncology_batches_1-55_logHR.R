...
if(scen_select=="small_n") analysis_functions_list<-c(
    analysis_functions_list,
    #gformula = analyse_oncology_gformula(B = 20,reps=10),
    gformula = analyse_oncology_gformula_unequal(B = 20,reps=10)
    )
...
if(scen_select=="IPCW_extra") {
  ...
  analysis_functions_list<-list(
    ...
    gformula = analyse_oncology_gformula(B = 20,reps=10),
    ipw = analyse_oncology_ipw(),
    rpsftm_rc_IPCW =  analyse_oncology_mixed(method="RPSFTM",recensor = TRUE,B = 100,trunc_weights = 5,use_censoring_IPW = TRUE,requ_n_cens = 5),
    tse_rc_IPCW =  analyse_oncology_mixed(method="TSE",recensor = TRUE,B = 100,trunc_weights = 5,use_censoring_IPW = TRUE,requ_n_cens = 5),
    gformula_IPCW = analyse_oncology_gformula_unequal(B = 20,reps=10,use_censoring_IPW=TRUE,requ_n_cens=5, trunc_weights=5),
