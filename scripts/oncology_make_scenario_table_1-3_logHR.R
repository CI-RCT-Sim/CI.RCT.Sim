...
#trueHR<-read.csv("scripts/trueHR_nsim20_n_ev10000.csv") #moved to oncology_run
trueHR<-trueHR_tab[,c("scen","trueHR")]
...
head(scen_tab)
dim(scen_tab)
scen_tab$true_eff[is.na(scen_tab$true_eff)]<-0
...
if(save_param_tab) {
  ...
  saveWorkbook(wb, paste(path,"/oncology_scenario_list.xlsx",sep=""), overwrite=TRUE)
