...
pre_my_summarise <- create_summarise_function(truth = summarise_estimator(
  est = HR,
...
my_analyse <- list(
  ...
  describe = function(condition, dat, fixed_objects = NULL) {
    tabulate_helper <- function(dat, var) {
      tmp <- list(sum(dat[, var]), sum(dat[dat$trt == 0, var]), sum(dat[dat$trt == 1, var]))
    ...
    if (!is.null(attr(dat, "followup"))) {
      ...
      result$study_time <- result$max_followup
    }
    if (hasName(dat, "ice")) {
...
my_summarise <- create_summarise_function(
  ...
  # rejection rates
  rpsftm_rc = summarise_test(alpha, name = "test"),
  rpsftm = summarise_test(alpha, name = "test"),
  tse_rc = summarise_test(alpha, name = "test"),
  tse = summarise_test(alpha, name = "test"),
  gformula = summarise_test(alpha, name = "test"),
  ipw = summarise_test(alpha, name = "test"),
  itt = summarise_test(alpha, name = "test"),
  cens = summarise_test(alpha, name = "test"),
