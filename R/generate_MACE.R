...
generate_one_tte2 <- function(X,Z,L,A,ID,hazard_disc,logit_withdraw,beta_tot_before,beta_tot_buffer,beta_tot_after,condition,fixed_objects=c(),no_withdraw=F) {
  ...
  if (is_withdraw & (dat_i$T_mace>dat_i$T_disc)) { 
    ...
    dat_temp$t_mace <- c(min(dat_i$T_mace,1))
...
calc_true_trt <- function(condition,fixed_objects) {
  ...
  dat_bind_bl <- pmap(cov_df,generate_one_tte2,condition=condition,no_withdraw=T,.progress = T)%>%bind_rows
